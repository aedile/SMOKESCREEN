/*
 * input.cpp - medal controls for Rally-X (held upright, like Pac-Man)
 *
 ...
 *
 * Rally-X is a four-way game, so only the dominant axis is ever reported.
 *
 *   twist left / right  -> left and right
 *   tip away / toward   -> up and down
 *   BOOT button         -> lay a smoke screen; hold 3 s for sound off and on
...
void input_update(rx_input_t *in)
{
    ...
    in->up     = (dir_y > 0) ? 1 : 0;
    in->down   = (dir_y < 0) ? 1 : 0;
    in->smoke  = st.boot ? 1 : 0;                    /* the button lays a smoke screen */
