 ...
 *
 * Rally-X is a four-way driving game: the car keeps going in whichever direction the stick is
 * held. Only the dominant axis is reported, so a diagonal tilt never produces two directions,
 * which is what the four-way gate on the cabinet did mechanically.
 *
 *   tilt left / right   -> steer left / right, the direction of gravity within the panel plane
 *   tilt away / toward  -> steer up / down, how far gravity leaves that plane
 *   BOOT button         -> lay a smoke screen
...
#define STEER_DEG      10.0f    /* tilt this far to steer */
...
void input_update(rx_input_t *in)
{
    ...
    if (imu_ok && now - imu_last_us >= IMU_PERIOD_US) {
        ...
        if (fabsf(roll) >= fabsf(pitch)) {
            if (roll > STEER_DEG) dir_x = +1; else if (roll < -STEER_DEG) dir_x = -1;
        } else {
            if (pitch > STEER_DEG) dir_y = +1; else if (pitch < -STEER_DEG) dir_y = -1;
        }
    }
    ...
    in->up    = (dir_y > 0) ? 1 : 0;
    in->smoke = boot ? 1 : 0;                     /* the button lays a smoke screen */
    in->down  = (dir_y < 0) ? 1 : 0;

    if (now - last_log >= 3000000) {
        last_log = now;
        ESP_LOGI(TAG, "imu %d  roll %+6.1f pitch %+6.1f -> U%d D%d L%d R%d S%d",
                 imu_ok, (double)dbg_roll, (double)dbg_pitch, in->up, in->down, in->left, in->right, in->smoke);
    }
}
